...
def get_train_lang(lang, max_length=500, n_samples=3):
    all_prompts = []
...
for lang in tqdm(languages[:]):
    ...
    # Define an empty list to store train_prompts
    df_ans = []
...
for i, l in enumerate(languages[:len(all_dfs)]):
    ...
    df_write.to_csv(f'llama_{l}_{max_length}_{n_samples}.txt', sep='\t', header=False)
